...
public class LongPollingTransport extends HttpClientTransport {
	...
	public SignalRFuture<Void> start(ConnectionBase connection,
			ConnectionType connectionType, DataResultCallback callback) {
		return poll(connection,
				connectionType == ConnectionType.InitialConnection ? "connect"
						: "reconnect", callback);
	}
	 ...
	 */
	private SignalRFuture<Void> poll(final ConnectionBase connection,
			final String connectionUrl, final DataResultCallback callback) {
		synchronized (mPollSync) {
			log("Start the communication with the server", LogLevel.Information);
			String url = connection.getUrl() + connectionUrl
			...
			final HttpConnectionFuture future = mHttpConnection.execute(get,
						...
						public void onResponse(Response response) {
							synchronized (mPollSync) {
								try {
									...
									log("Trigger onData with data: "
											+ responseData, LogLevel.Verbose);
									...
									if (!mConnectionFuture.isCancelled()
										...
										log("Continue polling",
												LogLevel.Verbose);
										mConnectionFuture.setFuture(poll(
				...
				public void onError(Throwable error) {
					synchronized (mPollSync) {
						if (connectionUrl.equals("poll")) {
							...
							mConnectionFuture.setFuture(poll(connection,
									"poll", callback));
						} else {
