...
public class HubConnection extends Connection {
	...
	private Map<String, Action<HubResult>> mCallbacks = Collections
			.synchronizedMap(new HashMap<String, Action<HubResult>>());
	private Map<String, HubProxy> mHubs = Collections
			.synchronizedMap(new HashMap<String, HubProxy>());
		...
		if (getState() == ConnectionState.Connected) {
			if (message.isJsonObject() && message.getAsJsonObject().has("I")) {
				...
				if (mCallbacks.containsKey(id)) {
					log("Get and remove callback with id: " + id,
							LogLevel.Verbose);
				...
				HubInvocation invocation = mGson.fromJson(message,
						HubInvocation.class);
				log("Getting HubInvocation from message", LogLevel.Verbose);

				String hubName = invocation.getHub().toLowerCase(
						Locale.getDefault());
				...
				if (mHubs.containsKey(hubName)) {
					...
					if (invocation.getState() != null) {
						for (String key : invocation.getState().keySet()) {
							...
							log("Setting state for hub: " + key + " -> "
									+ value, LogLevel.Verbose);
							hubProxy.setState(key, value);
						}
					}

					String eventName = invocation.getMethod().toLowerCase(
							Locale.getDefault());
					log("Invoking event: " + eventName + " with arguments "
							+ arrayToString(invocation.getArgs()),
							LogLevel.Verbose);
	...
	private void clearInvocationCallbacks(String error) {
		...
		for (String key : mCallbacks.keySet()) {
			try {
				log("Invoking callback with empty result: " + key,
						LogLevel.Verbose);
