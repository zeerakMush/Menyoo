...
public class Request {
	...
	public void setFormContent(List<Entry<String, String>> formValues) {
		...
		for (Entry<String, String> entry : formValues) {
			try {
				sb.append(String.format("%s=%s&", URLEncoder.encode(
						entry.getKey(), Constants.UTF8_NAME), URLEncoder
						.encode(entry.getValue(), Constants.UTF8_NAME)));
	...
	public void log(Logger logger) {
		if (logger != null) {
			...
			for (String key : mHeaders.keySet()) {
				logger.log("Header " + key + ": " + mHeaders.get(key),
						LogLevel.Verbose);
			}
