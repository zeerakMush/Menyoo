...
public class HubProxy {
	...
	private Map<String, Subscription> mSubscriptions = Collections
			.synchronizedMap(new HashMap<String, Subscription>());

	private Map<String, JsonElement> mState = Collections
			.synchronizedMap(new HashMap<String, JsonElement>());

	private Logger mLogger;

	private static final List<String> EXCLUDED_METHODS = Arrays
			.asList(new String[] { "equals", "getClass", "hashCode", "notify",
	...
	public Subscription subscribe(String eventName) {
		...
		if (mSubscriptions.containsKey(eventName)) {
			log("Adding event to existing subscription: " + eventName,
					LogLevel.Information);
			subscription = mSubscriptions.get(eventName);
		} else {
			log("Creating new subscription for: " + eventName,
					LogLevel.Information);
			...
			if (!EXCLUDED_METHODS.contains(method.getName())) {
				...
				subscription.addReceivedHandler(new Action<JsonElement[]>() {
					...
					public void run(JsonElement[] eventParameters)
							throws Exception {
						log("Handling dynamic subscription: "
								+ method.getName(), LogLevel.Verbose);
						Class<?>[] parameterTypes = method.getParameterTypes();
						if (parameterTypes.length != eventParameters.length) {
							throw new RuntimeException("The handler has "
									+ parameterTypes.length
									+ " parameters, but there are "
									+ eventParameters.length + " values.");
						}

						Object[] parameters = new Object[parameterTypes.length];

						for (int i = 0; i < eventParameters.length; i++) {
							parameters[i] = mConnection.getGson().fromJson(
									eventParameters[i], parameterTypes[i]);
						}
						method.setAccessible(true);
						log("Invoking method for dynamic subscription: "
								+ method.getName(), LogLevel.Verbose);
	 ...
	 */
	public <E> SignalRFuture<E> invoke(final Class<E> resultClass,
			final String method, Object... args) {
		if (method == null) {
		...
		final String callbackId = mConnection
				.registerCallback(new Action<HubResult>() {

					@Override
					public void run(HubResult result) {
						log("Executing invocation callback for: " + method,
								LogLevel.Information);
						if (result != null) {
							if (result.getError() != null) {
								if (result.isHubException()) {
									resultFuture.triggerError(new HubException(
											result.getError(), result
													.getErrorData()));
								} else {
									resultFuture.triggerError(new Exception(
											result.getError()));
								}
							} else {
								...
								try {
									if (result.getState() != null) {
										for (String key : result.getState()
												.keySet()) {
											setState(key, result.getState()
													.get(key));
										}
									}
									...
									if (result.getResult() != null
											&& resultClass != null) {
										log("Found result invoking method on hub: "
												+ result.getResult(),
												LogLevel.Information);
										resultObject = mConnection.getGson()
	...
	private <E1, E2, E3, E4, E5> void on(String eventName,
	...
	public <E1, E2, E3, E4, E5> void on(String eventName,
			final SubscriptionHandler5<E1, E2, E3, E4, E5> handler,
			Class<E1> parameter1, Class<E2> parameter2, Class<E3> parameter3,
			Class<E4> parameter4, Class<E5> parameter5) {
	...
	public <E1, E2, E3, E4> void on(String eventName,
	...
	public <E1, E2, E3> void on(String eventName,
	...
	public <E1, E2> void on(String eventName,
			final SubscriptionHandler2<E1, E2> handler, Class<E1> parameter1,
			Class<E2> parameter2) {
