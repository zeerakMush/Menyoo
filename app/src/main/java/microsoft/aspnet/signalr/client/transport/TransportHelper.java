...
public class TransportHelper {
	public static MessageResult processReceivedData(String data,
		...
		if (json.get("I") != null) {
			logger.log("Invoking message received with: " + json.toString(),
					LogLevel.Verbose);
			...
			if (json.get("G") != null) {
				...
				logger.log("Group token received: " + groupsToken,
						LogLevel.Verbose);
			...
			if (messages != null && messages.isJsonArray()) {
				...
				if (json.get("C") != null) {
					...
					logger.log("MessageId received: " + messageId,
							LogLevel.Verbose);
				...
				for (int i = 0; i < size; i++) {
					...
					logger.log("Invoking OnReceived with: " + processedMessage,
							LogLevel.Verbose);
			...
			if (json.get("S") != null) {
				if (json.get("S").getAsInt() == 1) {
					logger.log("Initialization message received",
	 ...
	 */
	public static String getReceiveQueryString(ClientTransport transport,
			ConnectionBase connection) {
		StringBuilder qsBuilder = new StringBuilder();

		qsBuilder.append("?transport=" + transport.getName()).append(
				"&connectionToken="
						+ urlEncode(connection.getConnectionToken()));

		qsBuilder.append("&connectionId="
				+ urlEncode(connection.getConnectionId()));

		if (connection.getMessageId() != null) {
			qsBuilder.append("&messageId="
					+ urlEncode(connection.getMessageId()));
		}

		if (connection.getGroupsToken() != null) {
			qsBuilder.append("&groupsToken="
					+ urlEncode(connection.getGroupsToken()));
		}
	...
	public static String getNegotiateQueryString(ConnectionBase connection) {
		StringBuilder qsBuilder = new StringBuilder();
		qsBuilder.append("?clientProtocol="
				+ urlEncode(Connection.PROTOCOL_VERSION.toString()));

		if (connection.getConnectionData() != null) {
			qsBuilder.append("&").append(
					"connectionData="
							+ urlEncode(connection.getConnectionData()));
		}
	 ...
	 * @return The querystring
	 */
	public static String getSendQueryString(ClientTransport transport,
			ConnectionBase connection) {
		StringBuilder qsBuilder = new StringBuilder();
		qsBuilder.append("?transport="
				+ TransportHelper.urlEncode(transport.getName()));

		qsBuilder.append("&connectionToken="
				+ TransportHelper.urlEncode(connection.getConnectionToken()));

		qsBuilder.append("&connectionId="
				+ TransportHelper.urlEncode(connection.getConnectionId()));

		if (connection.getConnectionData() != null) {
			qsBuilder
					.append("&connectionData="
							+ TransportHelper.urlEncode(connection
									.getConnectionData()));
		}
