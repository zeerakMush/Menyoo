...
public class ServerSentEventsTransport extends HttpClientTransport {
	...
	public SignalRFuture<Void> start(ConnectionBase connection,
		...
		mConnectionFuture = mHttpConnection.execute(get,
				new ResponseCallback() {
					...
					public void onResponse(Response response) {
						try {
							...
							log("Read the response content by line",
									LogLevel.Verbose);
							while ((line = response.readLine()) != null) {
								...
								if (currentData.endsWith(END_OF_SSE_MESSAGE)) {
									...
									log("Found new data: " + currentData,
											LogLevel.Verbose);
									if (currentData.equals(DATA_INITIALIZED)) {
										log("Initialization message found",
												LogLevel.Verbose);
									} else {
										String content = currentData.substring(
												SSE_DATA_PREFIX_LENGTH).trim();

										log("Trigger onData: " + content,
												LogLevel.Verbose);
							...
							// if the request finishes, it means the connection
							// was finalized
						} catch (Throwable e) {
