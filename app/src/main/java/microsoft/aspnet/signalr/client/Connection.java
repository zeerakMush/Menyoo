...
public class Connection implements ConnectionBase {
	...
	public Connection(String url, String queryString, Logger logger) {
		...
		log("Connection data: " + url + " - " + queryString == null ? ""
				: queryString, LogLevel.Verbose);
	...
	public SignalRFuture<Void> send(String data) {
		...
		SignalRFuture<Void> future = mTransport.send(this, data,
	 ...
	 */
	private void handleFutureError(SignalRFuture<?> future,
			final boolean mustCleanCurrentConnection) {
		final Connection that = this;
	...
	public SignalRFuture<Void> start(final ClientTransport transport) {
		synchronized (mStartLock) {
			...
			if (!changeState(ConnectionState.Disconnected,
				...
				log("Couldn't change state from disconnected to connecting.",
						LogLevel.Verbose);
				return mConnectionFuture;
			}

			log("Start the connection, using " + transport.getName()
					+ " transport", LogLevel.Information);
			...
			SignalRFuture<NegotiationResponse> negotiationFuture = transport
					.negotiate(this);
			...
			try {
				negotiationFuture.done(new Action<NegotiationResponse>() {
					...
					public void run(NegotiationResponse negotiationResponse)
						...
						if (!verifyProtocolVersion(negotiationResponse
								.getProtocolVersion())) {
							Exception err = new InvalidProtocolVersionException(
									negotiationResponse.getProtocolVersion());
							onError(err, true);
	 ...
	 */
	private boolean changeState(ConnectionState oldState,
	...
	public void stop() {
		synchronized (mStartLock) {
			...
			if (mState == ConnectionState.Disconnected) {
				log("Connection already in disconnected state. Exiting abort",
						LogLevel.Verbose);
				return;
			}
	...
	public void disconnect() {
		synchronized (mStateLock) {
			...
			if (mOnStateChanged != null) {
				try {
					mOnStateChanged.stateChanged(oldState,
	 ...
	 */
	private void startTransport(KeepAliveData keepAliveData,
		...
		synchronized (mStartLock) {
			...
			if (mTransport == null) {
				log("Transport is null. Exiting startTransport",
						LogLevel.Verbose);
				return;
			}
			...
			ConnectionType connectionType = isReconnecting ? ConnectionType.Reconnection
					: ConnectionType.InitialConnection;

			log("Starting transport for " + connectionType.toString(),
					LogLevel.Verbose);
			SignalRFuture<Void> future = mTransport.start(this, connectionType,
					new DataResultCallback() {
						@Override
						public void onData(String data) {
			...
			try {
				future.done(new Action<Void>() {
					...
					public void run(Void obj) throws Exception {
						synchronized (mStartLock) {
							log("Entered startLock after transport was started",
									LogLevel.Verbose);
							log("Current state: " + mState, LogLevel.Verbose);
							if (changeState(ConnectionState.Reconnecting,
							...
							} else if (changeState(ConnectionState.Connecting,
									ConnectionState.Connected)) {
	...
	protected void log(Throwable error) {
		mLogger.log(getSourceNameForLog() + " - Error: " + error.toString(),
				LogLevel.Critical);
	}
