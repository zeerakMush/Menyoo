...
public class HeartbeatMonitor {
	 ...
	 */
	public void start(KeepAliveData keepAliveData,
					...
					synchronized (mSync) {
						if (!mStopped) {
							if (connection.getState() == ConnectionState.Connected) {
								long lastKeepAlive = mKeepAliveData
										.getLastKeepAlive();
								long timeElapsed = Calendar.getInstance()
										.getTimeInMillis() - lastKeepAlive;
								...
								} else if (timeElapsed >= mKeepAliveData
										.getTimeoutWarning()) {
									if (!mHasBeenWarned) {
	...
	public void beat() {
		synchronized (mSync) {
			if (mKeepAliveData != null) {
				mKeepAliveData.setLastKeepAlive(Calendar.getInstance()
						.getTimeInMillis());
			}
		}
	}
