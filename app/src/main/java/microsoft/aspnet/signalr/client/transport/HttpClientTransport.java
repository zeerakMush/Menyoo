...
public abstract class HttpClientTransport implements ClientTransport {
	...
	@Override
	public SignalRFuture<NegotiationResponse> negotiate(
		...
		String url = connection.getUrl() + "negotiate"
				+ TransportHelper.getNegotiateQueryString(connection);
		...
		HttpConnectionFuture connectionFuture = mHttpConnection.execute(get,
				new ResponseCallback() {
					...
					public void onResponse(Response response) {
						try {
							...
							log("Read response data to the end",
									LogLevel.Verbose);
							String negotiationContent = response.readToEnd();

							log("Trigger onSuccess with negotiation data: "
									+ negotiationContent, LogLevel.Verbose);
							negotiationFuture
									.setResult(new NegotiationResponse(
											negotiationContent, connection
													.getJsonParser()));

						} catch (Throwable e) {
							log(e);
							negotiationFuture
									.triggerError(new NegotiationException(
	...
	public SignalRFuture<Void> send(ConnectionBase connection, String data,
		...
		try {
			log("Start sending data to the server: " + data,
					LogLevel.Information);

			Request post = new Request(Constants.HTTP_POST);
			post.setFormContent("data", data);
			post.setUrl(connection.getUrl() + "send"
					+ TransportHelper.getSendQueryString(this, connection));
			...
			log("Execute the request", LogLevel.Verbose);
			HttpConnectionFuture future = mHttpConnection.execute(post,
					new ResponseCallback() {
						...
						public void onResponse(Response response)
							...
							if (data != null) {
								log("Trigger onData with data: " + data,
										LogLevel.Verbose);
			...
			if (!mStartedAbort) {
				...
				try {
					String url = connection.getUrl()
							+ "abort"
							+ TransportHelper.getSendQueryString(this,
									connection);
					...
					mAbortFuture = mHttpConnection.execute(post,
							new ResponseCallback() {
	...
	protected void throwOnInvalidStatusCode(Response response)
			throws InvalidHttpStatusCodeException {
		if (response.getStatus() < 200 || response.getStatus() > 299) {
			...
			throw new InvalidHttpStatusCodeException(response.getStatus(),
					responseContent, headersString.toString());
		}
	...
	protected void log(Throwable error) {
		mLogger.log(getName() + " - Error: " + error.toString(),
				LogLevel.Critical);
	}
